...
void assignFileNDataInMemo(VDisk *vd, FILE *f, const char *fNam, const char *fData){
  ...
  for(size_t i = 0; i < blockNeeded; i++){
    ...
    //File line
    fseek(f, curIdx * sizeof(Blocks), SEEK_SET);
    fwrite(&vd->blocks[curIdx], sizeof(Blocks), 1, f);
  ...
  */
  ...
  for(size_t i = 0; i < BLOCK_N; i++){
    fseek(f, i * sizeof(Blocks), SEEK_SET);
    fwrite(&vd->blocks[i], sizeof(Blocks), 1, f);
  }
}

void doCreateFile(VDisk *vd, FILE *f){
...
(Press '~' for to save file.):\n");
  ...
  assignFileNDataInMemo(vd, f, file_Name, data);
  
  free(data);
  data = NULL; 
}

char* appendBuffer(char *buffer, size_t *bufSize, const char *nxtBCnt){
  size_t currentSize = strlen(buffer);
  size_t nxtBCS = strlen(nxtBCnt);
  size_t neededSize = currentSize + nxtBCS + 1;
  if(neededSize > *bufSize){
    char *temp = realloc(buffer, neededSize);
    if(!temp){
  ...
  memcpy(buffer + currentSize, nxtBCnt, nxtBCS + 1);
  return buffer;
}

void doSearchFile(VDisk *vd){
  char fileName[64];
  printf("Enter the file name:\n");
  fflush(stdout);
  fgets(fileName, sizeof(fileName), stdin);
  fileName[strcspn(fileName, "\n")] = '\0';
  printf("File name you entered: %s\n", fileName);

  memcpy(&diskInfo, vd->blocks[0].content, sizeof(DiskInfo));
  printf("FileCount %zu\n", diskInfo.fCt);
  
  int64_t i = 0;
  size_t fcount = 0;
  int ffound = -1;
  while(i != -1  && fcount < diskInfo.fCt){
    /*
      we are not checking blocktype
      because we knew that blocks[0] is META_BLOCK
      and nextBCHain is not -1 if there any other META_BLOCKs exist 
    */
    //loop or hop to find the file
    FileInfo *finfo = (i == 0) ?
      (FileInfo *) (vd->blocks[i].content + sizeof(DiskInfo)):
      (FileInfo *) (vd->blocks[i].content);
    //We also knew that 1 META_BLOCK can only contain 3 fileInfo
    for(size_t j = 0; j < 3 && (fcount < diskInfo.fCt); j++){
      printf("FileName: %zu = %s\n", j, finfo[j].fileNam);
      if(strncmp(finfo[j].fileNam, fileName, 63) == 0){
	ffound = 1;
	
	printf("Yes we have %s file.\nDo you wanna open it?\n", fileName);

	int userChoice;
	puts("For 'Yes'/'No' Press 1 or -1 respectively.");
	fflush(stdout);
	scanf("%d", &userChoice);
	getchar();

	//Here comes file open logic only for read
	//for safe appraoch let's use buffer
	//File size may vary so we need dynamic memory (heap)
	//We are only providing content part hiding the all abstraction(like metadata, fileInfo and etc.)
	//for to get block count we need to check for all the block's neckBChain value then for add that index block's content in buffer
	if( userChoice == 1){
	  uint64_t fileIdx = finfo[j].initBkIdx;
	  printf("%" PRId64, fileIdx);
	  size_t bufSize = strlen(vd->blocks[fileIdx].content) + 1;
	  char *buffer = (char*)malloc(bufSize);
	  if(!buffer){
	    perror("Memory allocation for buffer is failed!\n");
	    return;
	  }
	  strcpy(buffer, vd->blocks[fileIdx].content);
	  
	  for(int64_t b = vd->blocks[fileIdx].nextBChain; b != -1;){
	    // Ensure block content is null-terminated
	    vd->blocks[b].content[strlen(vd->blocks[b].content)] = '\0';
	    
	    buffer = appendBuffer(buffer, &bufSize, vd->blocks[b].content);
	    if(!buffer){
	      perror("ERROR: Read Buffer is empty.\n");
	      return;
	    }
	    b = vd->blocks[b].nextBChain;
	  }
	  printf("File Content:\n %s\n", buffer);
	  free(buffer);
	  ...
	  /*
	    To Do: Next Idea:
	    For this section is to link it with update or edit file Operation
	    (Problem we'll gonna face:
	    1. How to remove or free space in metablock so we can restore the update file with it's new meta info
	    2. this also solve the delete operation problem
	   */
  ...
  if(ffound == -1){
    printf("We don't have %s file.\n", fileName);
  }  
}

void loadDisk(VDisk *vd, FILE *f){
  for(size_t i = 0; i < BLOCK_N; i++){
    fread(&vd->blocks[i], sizeof(Blocks), 1, f);
  }
}
...
int main(){
  ...
  if(!f){
    fprintf(stderr, "ERROR: %s file is not created!\n", fileName);
    return 1;
  }
  ...
  while(1){
    ...
    printf("=======Press 2. Search File ========\n");
    printf("=====Press 3. Exit Appication ======\n");
